...
from feature_engineering import preprocessing_per_fold

import warnings
warnings.simplefilter("ignore")

def evaluate(CFG, fs: List[str], train: pd.DataFrame):
    """特徴量選択のための評価関数

    Args:
        CFG: config
        fs (List[str]): 学習に用いる特徴量
        train (pd.DataFrame): 学習用データ
    ...
    for fold in range(CFG.n_splits):
        X_train, X_valid = preprocessing_per_fold(CFG, train, test = None, fold=fold, predict = False)
        # params
    ...
    score = get_score(y_true=oof_df["price"], y_pred=oof_df["pred"])
    return score